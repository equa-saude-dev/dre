        ...
        <section className="tab-panel g1 active">
          <div className="panel"><div className="ph"><h2>Premissas da rodada</h2>{premDirty && <span style={{ fontSize: '.78rem', color: 'var(--war)', fontWeight: 600 }}>⚠ Alterações pendentes — clique em Aplicar</span>}</div><div className="pb">
            <div className="fields sub4">
              <PHintField label="Captação (R$)" field="captacao" value={getPrem('captacao') as number} onChange={v => setPremField('captacao', Number(v))} hint={FIELD_HINTS.captacao} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
              <PHintField label="Equity %" field="equity" step={0.1} value={getPrem('equity') as number} onChange={v => setPremField('equity', Number(v))} hint={FIELD_HINTS.equity} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
              <PHintField label="Hospitais alvo" field="hFim" value={getPrem('hFim') as number} onChange={v => setPremField('hFim', Number(v))} hint={FIELD_HINTS.hFim} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
              <PHintField label="Prazo total (meses)" field="mesesPlan" min={6} max={36} value={getPrem('mesesPlan') as number} onChange={v => setPremField('mesesPlan', Math.min(36, Math.max(6, Number(v))))} hint={FIELD_HINTS.mesesPlan} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
            </div>
            <div className="fields sub2" style={{ marginTop: '.875rem' }}><PHintField label="Início 1ª receita (mês)" field="inicioRec" value={getPrem('inicioRec') as number} onChange={v => setPremField('inicioRec', Number(v))} hint={FIELD_HINTS.inicioRec} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} /></div>
            <span className="lbl" style={{ marginTop: '1.25rem' }}>🏥 Receita Core — Glosas</span>
            <div className="fields sub3">
              <PHintField label="Subscription / contrato (R$)" field="sub" value={getPrem('sub') as number} onChange={v => setPremField('sub', Number(v))} hint={FIELD_HINTS.sub} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
              <PHintField label="Performance Fee / contrato (R$)" field="perf" value={getPrem('perf') as number} onChange={v => setPremField('perf', Number(v))} hint={FIELD_HINTS.perf} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
              <div className="field"><label>Receita piloto / mês (R$) <InfoBtn field="piloto" /></label><input type="number" value={getPrem('piloto') as number} onChange={e => setPremField('piloto', Number(e.target.value))} /><small>0 = piloto gratuito</small><TooltipBox field="piloto" /></div>
            </div>
            <span className="lbl" style={{ marginTop: '1.25rem' }}>⚡ Equa Pay — Antecipação</span>
            <div className="fields sub2">
              <PHintField label="Faturamento do hospital / mês (R$)" field="equaPayVol" value={getPrem('equaPayVol') as number} onChange={v => setPremField('equaPayVol', Number(v))} hint={FIELD_HINTS.equaPayVol} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
              <div className="field"><label>Taxa de antecipação (%) <InfoBtn field="equaPayTaxa" /></label><input type="number" step={0.1} value={getPrem('equaPayTaxa') as number} onChange={e => setPremField('equaPayTaxa', Number(e.target.value))} /><small>Cobrada sobre o faturamento</small><TooltipBox field="equaPayTaxa" /></div>
            </div>
            <span className="lbl" style={{ marginTop: '1.25rem' }}>📊 Revenue Share</span>
            <div className="fields sub3">
              <div className="field"><label>Revenue Share (%) <InfoBtn field="revSharePct" /></label><input type="number" step={0.5} value={getPrem('revSharePct') as number} onChange={e => setPremField('revSharePct', Number(e.target.value))} /><small>% sobre o faturamento antecipado</small><TooltipBox field="revSharePct" /></div>
              <PHintField label="Faturamento do hospital / hosp / mês (R$)" field="revShareBase" value={getPrem('revShareBase') as number} onChange={v => setPremField('revShareBase', Number(v))} hint={FIELD_HINTS.revShareBase} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
              <PHintField label="Mês de início (Equa Pay + Rev. Share)" field="revShareIni" value={getPrem('revShareIni') as number} onChange={v => setPremField('revShareIni', Number(v))} hint={FIELD_HINTS.revShareIni} tooltip={tooltip} setTooltip={setTooltip} InfoBtn={InfoBtn} TooltipBox={TooltipBox} />
            </div>
          ...
          <div className="panel"><div className="ph"><h2>Estrutura de Custos</h2></div><div className="pb">
            {AREA_LIST.map(area => {
              ...
              return (<div key={area} className="custo-panel"><div className="custo-header"><div><span className="custo-area-name">{AREA_LABELS[area]}</span><div className="custo-summary"><span className="pill">{BRL(total)}/mês</span></div></div><button className="btn-sm" onClick={() => addCost(area)}>+ Item</button></div><div className="custo-body"><table className="cost-table"><thead><tr><th>Categoria</th><th>Descrição</th><th>R$/mês</th><th>Mês ini.</th><th>Mês fim</th><th></th></tr></thead><tbody>
                {items.map(c => (<tr key={c.id}><td><select value={c.cat} onChange={e => updCost(area, c.id, { cat: e.target.value as any })}>{Object.keys(CAT_LABELS).map(cat => <option key={cat} value={cat}>{CAT_LABELS[cat]}</option>)}</select></td><td><input value={c.desc} onChange={e => updCost(area, c.id, { desc: e.target.value })} /></td><td><input type="number" value={c.monthly} onChange={e => updCost(area, c.id, { monthly: Number(e.target.value) })} /></td><td><input type="number" value={c.startM} onChange={e => updCost(area, c.id, { startM: Number(e.target.value) })} /></td><td><input type="number" value={c.endM} onChange={e => updCost(area, c.id, { endM: Number(e.target.value) })} /></td><td><button className="btn-sm btn-danger" onClick={() => delCost(area, c.id)}>✕</button></td></tr>))}
              </tbody></table></div></div>);
            })}
          </div></div>
        </section>
            ...
            {state.phases.map(ph => (
              <div key={ph.id} className="okr-card">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto auto', gap: '.75rem', marginBottom: '.75rem' }}>
                  <input value={ph.name} onChange={e => updPhase(ph.id, { name: e.target.value })} /><input value={ph.objective} onChange={e => updPhase(ph.id, { objective: e.target.value })} /><input type="number" value={ph.startM} style={{ width: '60px' }} onChange={e => updPhase(ph.id, { startM: Number(e.target.value) })} /><input type="number" value={ph.endM} style={{ width: '60px' }} onChange={e => updPhase(ph.id, { endM: Number(e.target.value) })} />
                </div>
                <input value={ph.kr} style={{ marginBottom: '.75rem' }} onChange={e => updPhase(ph.id, { kr: e.target.value })} placeholder="Key Result" />
                <table className="init-table"><tbody>{ph.initiatives.map(ini => (<tr key={ini.id}><td><input value={ini.name} onChange={e => updInitiative(ph.id, ini.id, { name: e.target.value })} /></td><td><select value={ini.area} onChange={e => updInitiative(ph.id, ini.id, { area: e.target.value })}>{Object.keys(AREA_LABELS).map(a => <option key={a} value={a}>{AREA_LABELS[a]}</option>)}</select></td><td><input type="number" value={ini.pct} style={{ width: '50px' }} onChange={e => updInitiative(ph.id, ini.id, { pct: Number(e.target.value) })} /></td><td><button className="btn-sm btn-danger" onClick={() => delInitiative(ph.id, ini.id)}>✕</button></td></tr>))}</tbody></table>
          ...
          <div className="panel"><div className="ph"><h2>Cenários</h2></div><div className="pb">
            <div className="tw"><table><thead><tr><th>Cenário</th><th>Captação</th><th>Equity</th><th>Hosp.</th><th className="r">Caixa final</th><th className="r">Resultado</th><th></th></tr></thead><tbody>
              {state.scenarios.map(s => {
                ...
                return (<tr key={s.id}><td><input value={s.name} onChange={e => updScenario(s.id, { name: e.target.value })} /></td><td><input type="number" value={s.cap} onChange={e => updScenario(s.id, { cap: Number(e.target.value) })} /></td><td><input type="number" value={s.eq} onChange={e => updScenario(s.id, { eq: Number(e.target.value) })} /></td><td><input type="number" value={s.hFim} onChange={e => updScenario(s.id, { hFim: Number(e.target.value) })} /></td><td className="r">{BRL(r.caixaFinal)}</td><td className="r">{BRL(r.resultado)}</td><td><button className="btn-sm btn-danger" onClick={() => delScenario(s.id)}>✕</button></td></tr>);
              })}
            </tbody></table></div>
...
function PHintField({ label, field, value, onChange, hint, tooltip, setTooltip, InfoBtn, TooltipBox, min, max, step }: any) {
  return (<div className="field"><label>{label} <InfoBtn field={field} /></label><input type="number" value={value} min={min} max={max} step={step} onChange={e => onChange(e.target.value)} /><TooltipBox field={field} /></div>);
}

function DRERow({ label, data, k, brl, bold, neg, indent, color, subtotal, dtotal }: any) {
  const tot = data.reduce((a, d) => a + (d[k] as number), 0);
  const style: any = { ...(bold ? { fontWeight: 700 } : {}), ...(color ? { color } : {}), ...(indent ? { paddingLeft: '1.2rem' } : {}) };
  return (<tr className={dtotal ? 'total' : subtotal ? 'subtotal' : ''}><td style={style}>{label}</td>{data.map((d: any) => <td key={d.m} className="r" style={style}>{brl ? BRL(neg ? -(d[k]) : d[k]) : d[k]}</td>)}<td className="r bold" style={style}>{brl ? BRL(neg ? -tot : tot) : '—'}</td></tr>);
}
